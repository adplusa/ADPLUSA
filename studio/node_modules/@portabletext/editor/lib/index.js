...
import { useEffect, createContext, useContext, useState, useRef, startTransition, Component, useMemo, useCallback, forwardRef, useImperativeHandle } from "react";
...
import { defineBehavior, forward, raise as raise$1 } from "./behaviors/index.js";
import uniq from "lodash/uniq.js";
import { Subject } from "rxjs";
import { setup, stateIn, fromCallback, assign, enqueueActions, emit, assertEvent, and, not, raise, createActor } from "xstate";
...
}, debug$9 = debugWithName("component:PortableTextEditor:SlateContainer"), slateEditors = /* @__PURE__ */ new WeakMap();
function createSlateEditor(config) {
  const existingSlateEditor = slateEditors.get(config.editorActor);
  if (existingSlateEditor)
    return debug$9("Reusing existing Slate editor instance", config.editorActor.id), existingSlateEditor;
  debug$9("Creating new Slate editor instance", config.editorActor.id);
...
const debug$7 = debugWithName("sync machine"), syncValueCallback = ({
  sendBack,
  input
}) => {
  updateValue({
  ...
  states: {
    idle: {
      entry: [() => {
        debug$7("entry: syncing->idle");
      }],
      exit: [() => {
        debug$7("exit: syncing->idle");
      }],
      on: {
        "update value": [{
          ...
          actions: [() => {
            debug$7("no new value \u2013 setting initial value as synced");
          }, "assign initial value synced", "emit done syncing value"]
        }, {
          actions: [() => {
            debug$7("no new value and initial value already synced");
          }]
        }]
      }
    },
    busy: {
      entry: [() => {
        debug$7("entry: syncing->busy");
      }],
      exit: [() => {
        debug$7("exit: syncing->busy");
      }],
      after: {
        1e3: [{
          guard: "is busy",
    ...
    syncing: {
      entry: [() => {
        debug$7("entry: syncing->syncing");
      }, "emit syncing value"],
      exit: [() => {
        debug$7("exit: syncing->syncing");
      }, "emit done syncing value"],
      invoke: {
        src: "sync value",
        id: "sync value",
...
async function updateValue({
  ...
  if ((!value || value.length === 0) && (debug$7("Value is empty"), Editor.withoutNormalizing(slateEditor, () => {
  ...
  if (!isValid) {
    debug$7("Invalid value, returning"), doneSyncing = !0, sendBack({
      type: "done syncing",
      value
    });
    return;
  ...
  if (isChanged) {
    debug$7("Server value changed, syncing editor");
    try {
    ...
    debug$7("Server value and editor value is equal, no need to sync.");
  doneSyncing = !0, sendBack({
    type: "done syncing",
    value
  });
    ...
    withRemoteChanges(slateEditor, () => {
      withoutPatching(slateEditor, () => {
        if (hasChanges && blockValid) {
          ...
          })), validation.valid || validation.resolution?.autoResolve ? (oldBlock._key === currentBlock._key ? (debug$7.enabled && debug$7("Updating block", oldBlock, currentBlock), _updateBlock(slateEditor, currentBlock, oldBlock, currentBlockIndex)) : (debug$7.enabled && debug$7("Replacing block", oldBlock, currentBlock), _replaceBlock(slateEditor, currentBlock, currentBlockIndex)), blockChanged = !0) : (sendBack({
            type: "invalid value",
        ...
        if (!oldBlock && blockValid) {
          const validationValue = [value[currentBlockIndex]], validation = validateValue(validationValue, context.schema, context.keyGenerator);
          debug$7.enabled && debug$7("Validating and inserting new block in the end of the value", currentBlock), validation.valid || validation.resolution?.autoResolve ? Transforms.insertNodes(slateEditor, currentBlock, {
            at: [currentBlockIndex]
          }) : (debug$7("Invalid", validation), sendBack({
            type: "invalid value",
            resolution: validation.resolution,
            value
          }), blockValid = !1);
...
function _updateBlock(slateEditor, currentBlock, oldBlock, currentBlockIndex) {
  if (Transforms.setNodes(slateEditor, currentBlock, {
    ...
    currentBlock.children.length < oldBlockChildrenLength && Array.from(Array(oldBlockChildrenLength - currentBlock.children.length)).forEach((_, index) => {
      ...
      childIndex > 0 && (debug$7("Removing child"), Transforms.removeNodes(slateEditor, {
      ...
      if (isChildChanged)
        if (currentBlockChild._key === oldBlockChild?._key) {
          debug$7("Updating changed child", currentBlockChild, oldBlockChild), Transforms.setNodes(slateEditor, currentBlockChild, {
            at: path
          });
          ...
          isSpanNode && isTextChanged ? (oldBlockChild.text.length > 0 && Transforms.delete(slateEditor, {
            ...
            at: path
          }), slateEditor.onChange()) : isSpanNode || (debug$7("Updating changed inline object child", currentBlockChild), Transforms.setNodes(slateEditor, {
            _key: VOID_CHILD_KEY
          }, {
            at: [...path, 0],
            voids: !0
          }));
        } else oldBlockChild ? (debug$7("Replacing child", currentBlockChild), Transforms.removeNodes(slateEditor, {
          at: [currentBlockIndex, currentBlockChildIndex]
        }), Transforms.insertNodes(slateEditor, currentBlockChild, {
          at: [currentBlockIndex, currentBlockChildIndex]
        }), slateEditor.onChange()) : oldBlockChild || (debug$7("Inserting new child", currentBlockChild), Transforms.insertNodes(slateEditor, currentBlockChild, {
          at: [currentBlockIndex, currentBlockChildIndex]
        }), slateEditor.onChange());
    });
  }
}
const debug$6 = debugWithName("createInternalEditor");
...
function createInternalEditor(editorActor) {
  ...
  return mutationActor.start(), syncActor.start(), {
...
const actors = /* @__PURE__ */ new WeakMap();
function createActors(config) {
  const existingActor = actors.get(config.editorActor);
  if (existingActor)
    return debug$6("Reusing existing actors"), existingActor;
  debug$6("Creating new actors");
  ...
  return mutationActor.on("*", (event) => {
    ...
    event.type === "read only" && syncActor.send({
      type: "update readOnly",
      readOnly: !0
    }), event.type === "editable" && syncActor.send({
      type: "update readOnly",
      readOnly: !1
...
const arrowDownOnLonelyBlockObject = defineBehavior({
  ...
  actions: [(_, {
    selectedListBlocks
  }) => selectedListBlocks.map((selectedListBlock) => raise$1({
    type: "block.set",
    props: {
  ...
  actions: [({
    event
  }) => event.blocks.map((block, index) => raise$1({
    type: "insert.block",
    block,
  ...
  actions: [({
    event
  }) => event.blocks.map((block) => raise$1({
    type: "insert.block",
    block,
    placement: "after",
    select: "end"
  ...
  actions: [({
    ...
    focusTextBlock
  }) => event.blocks.length === 1 ? [raise$1({
    type: "insert.block",
    block: event.blocks[0],
    placement: "auto",
    select: "end"
  })] : isEmptyTextBlock(snapshot.context, focusTextBlock.node) ? event.blocks.map((block, index) => raise$1({
    type: "insert.block",
    block,
    placement: index === 0 ? "auto" : "after",
    select: "end"
  })) : event.blocks.flatMap((block, index) => index === 0 ? [raise$1({
    type: "split"
  }), raise$1({
    ...
    placement: "auto",
    select: "end"
  })] : index === event.blocks.length - 1 ? [raise$1({
...
`
  ...
  actions: [({
    ...
    selectedTextBlocks
  }) => selectedTextBlocks.map((block) => raise$1({
    type: "block.set",
    at: block.path,
    props: {
      level: 1,
  ...
  actions: [({
    event
  }, serializeEvents) => serializeEvents.map((serializeEvent) => raise$1({
    ...serializeEvent,
    originEvent: event.originEvent
  }))]
}), abstractBehaviors = [
  ...
  defineBehavior({
    ...
    actions: [({
      ...
      dragOrigin,
      originEvent
    }) => [...draggingEntireBlocks ? draggedBlocks.map((block) => raise$1({
      type: "delete.block",
      at: block.path
    })) : [raise$1({
      type: "delete",
  ...
  defineBehavior({
    ...
    actions: [(_, {
      ...
      activeDecorators,
      textRuns
    }) => textRuns.flatMap((textRun, index) => index !== textRuns.length - 1 ? [raise$1({
      type: "insert.span",
      text: textRun,
...
class PortableTextEditor extends Component {
  ...
  constructor(props) {
    if (super(props), props.editor)
      ...
      editorActor.start(), editorActor.on("*", (event) => {
        const change = eventToChange(event);
        change && (props.onChange(change), this.change$.next(change));
      }), this.editor = createInternalEditor(editorActor), this.schemaTypes = legacySchema;
    }
