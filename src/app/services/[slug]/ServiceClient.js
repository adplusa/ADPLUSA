...
export default function ServiceClient({ service, otherServices, slug }) {
    ...
    const slidesToShow = otherServices.length > 0 ? [...otherServices, ...otherServices] : [];
    ...
    return (
        <div className="service-detail-container">
            ...
            {service.bannerImage?.url && (
                <section className="service-banner" style={{ backgroundImage: `url(${service.bannerImage.url})`, backgroundSize: "cover", backgroundPosition: "center" }} />
            )}
                        ...
                        <div key={i} className="service-info-df">
                            <div className="service-left">
                                ...
                                {item.link && (
                                    <div className="service-link-container">
                                        {item.isExternal ? (
                                            <a href={item.link} target="_blank" rel="noopener noreferrer" className="service-link-btn">Learn More →</a>
                                        ) : (
                                            <Link href={item.link} className="service-link-btn">Learn More →</Link>
                                        )}
                                    </div>
                                )}
                            </div>
                            {item.image?.url && (
                                <div className="service-right">
                                    <Image src={item.image.url} width={600} height={400} unoptimized alt={item.title || "Service Image"} />
                                </div>
                            )}
                        </div>
                    ))}
                ...
                <div className="key-container">
                    ...
                    <div className="key-cards-container">
                        {service.features.map((feature, i) => (
                            <div className="key-card" key={i}>
                                <div className="key-asterisk">*</div>
                                <h3 className="key-card-title">{feature.title}</h3>
                                <p className="key-card-description">{feature.description}</p>
                            </div>
                        ))}
                    </div>
                </div>
                ...
                <div className="professionals-section-internals">
                    ...
                    <div className="carousel-container-internals" onMouseEnter={pauseAutoPlay} onMouseLeave={resumeAutoPlay}>
                        <div
                            ...
                            ref={slideRef}
                            style={{ transform: `translateX(-${currentIndex * (isMobile ? 100 : 25)}%)`, transition: isDragging ? "none" : "transform 0.5s ease-in-out", cursor: isDragging ? "grabbing" : "grab", userSelect: "none" }}
                            onMouseDown={handleDragStart}
                            onTouchStart={handleDragStart}
                        >
                            {slidesToShow.map((svc, i) => (
                                <Link key={i} href={`/services/${svc.slug}`} id="redirection-service">
                                    <div className="carousel-slide-internals">
                                        <div className="professional-card-internals">
                                            <div className="image-container-internals">
                                                {svc.bannerImage?.url || svc.image?.url ? (
                                                    <Image src={svc.bannerImage?.url || svc.image?.url} alt={svc.title} width={300} height={200} unoptimized draggable={false} style={{ pointerEvents: isDragging ? "none" : "auto", userSelect: "none" }} />
                                                ) : (
                                            ...
                                            <h3 style={{ userSelect: "none" }}>{svc.title}</h3>
                                        </div>
                                    </div>
                                </Link>
                            ))}
