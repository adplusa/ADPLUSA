 ...
 * Fetches service by slug from the custom CMS
...
export default function ServiceDetail() {
  ...
  useEffect(() => {
    ...
    const fetchData = async () => {
      ...
      try {
        ...
        // Fetch other services for the carousel
        ...
        if (serviceData.seoTitle) {
          ...
          document.title = `${serviceData.title} | ADPL Consulting`;
        }

        if (serviceData.seoDescription) {
          let metaDesc = document.querySelector("meta[name='description']");
          if (metaDesc) {
            metaDesc.setAttribute("content", serviceData.seoDescription);
          } else {
            metaDesc = document.createElement("meta");
            metaDesc.name = "description";
            metaDesc.content = serviceData.seoDescription;
  ...
  useEffect(() => {
    if (otherServices.length > 0 && !isDragging) {
      startAutoPlay();
    }
    return () => stopAutoPlay();
  ...
  // Loading state
  if (isLoading) {
    return <Loading text="Loading Service" fullScreen={true} />;
  }
  ...
  return (
    <div className="service-detail-container">
                ...
                {item.link && (
                  <div className="service-link-container">
                    {item.isExternal ? (
                      ...
                      <Link href={item.link} className="service-link-btn">
                        Learn More →
                      </Link>
                    )}
                  </div>
      ...
      {/* Explore More Services Carousel */}
                    ...
                    <div className="professional-card-internals">
                      <div className="image-container-internals">
                        {svc.bannerImage?.url || svc.image?.url ? (
                          <Image
                            src={svc.bannerImage?.url || svc.image?.url}
                            alt={svc.title}
                            width={300}
                            height={200}
                            unoptimized
          ...
          <div className="enquiry-container" onClick={(e) => e.stopPropagation()}>
            <div className="enquiry-box">
              ...
              <form>
                ...
                <textarea placeholder="Query" className="form-input" rows="3"></textarea>
