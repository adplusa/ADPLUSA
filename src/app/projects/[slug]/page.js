  ...
  useEffect(() => {
    ...
    const fetchData = async () => {
      ...
      try {
        ...
        // Set SEO meta tags
        if (projectData.seoTitle) {
          document.title = projectData.seoTitle;
        } else if (projectData.title) {
          document.title = `${projectData.title} | ADPL Consulting`;
        }

        if (projectData.seoDescription) {
          let metaDesc = document.querySelector("meta[name='description']");
          if (metaDesc) {
            metaDesc.setAttribute("content", projectData.seoDescription);
          } else {
            metaDesc = document.createElement("meta");
            metaDesc.name = "description";
            metaDesc.content = projectData.seoDescription;
      ...
      <div className="internal-section-one">
        <div className="internal-section-one-top">
          ...
          {(project.mainImage?.url || project.images?.[0]?.url) && (
            <Image
              src={project.mainImage?.url || project.images?.[0]?.url}
              alt={project.mainImage?.alt || project.images?.[0]?.alt || project.title}
              width={1200}
              height={600}
              unoptimized
              priority
          ...
          <div className="internal-section-one-bottom-left">
            ...
            {project.moreContent && (
              <div
                ...
                dangerouslySetInnerHTML={{ __html: project.moreContent }}
              />
            )}
          </div>
                    ...
                    <div className="professional-card-internals" id="project-caraousel">
                      <div className="image-container-internals">
                        {(proj.mainImage?.url || proj.images?.[0]?.url) ? (
                          <Image
                            src={proj.mainImage?.url || proj.images?.[0]?.url}
                            alt={proj.mainImage?.alt || proj.images?.[0]?.alt || proj.title}
                            width={300}
                            height={200}
                            unoptimized
                            draggable={false}
