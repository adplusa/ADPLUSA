...
import "./internal-eighteen.css";
import urlFor from "@/app/helpers/sanity";
import Link from "next/link";

const InternalEighteen = () => {
    ...
    const fetchData = async () => {
      try {
        const fetched = await client.fetch(
          `*[_type == "projectInternalPageEighteen"]`
        );
        setData(fetched);

        // ✅ Fetch other services for carousel
  ...
  const handleDragMove = useCallback(
    (e) => {
      ...
      slideRef.current.style.transform = `translateX(${-currentIndex * slideWidth + dragOffset}%)`;
    },
...
export default InternalEighteen;
