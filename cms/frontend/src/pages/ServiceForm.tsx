...
export default function ServiceForm() {
  ...
  const { fields: servicesListFields, append: appendService, remove: removeService } = useFieldArray({
    control,
    name: 'servicesList',
  });

  const { fields: keyActivitiesFields, append: appendActivity, remove: removeActivity } = useFieldArray({
    control,
    name: 'keyActivities',
  });
  ...
  const onSubmit = useCallback(async (data: ServiceFormData) => {
  ...
  const tabs: FormTab[] = [
    ...
    { id: 'services', label: 'Services List', icon: '📋', count: servicesListFields.length },
    { id: 'activities', label: 'Key Activities', icon: '⚡', count: keyActivitiesFields.length },
    { id: 'features', label: 'Features', icon: '✨', count: featuresFields.length },
  ...
  return (
    <>
      ...
      <FormWrapper
        ...
        subtitle={isEditMode ? 'Update the service details below' : 'Fill in the details to create a new service'}
      ...
      >
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
              ...
              >
                <input
                  ...
                  type="text"
                  {...register('title')}
                  className={`w-full px-4 py-2.5 border rounded-lg focus:ring-2 focus:ring-indigo-500 focus:border-indigo-500 transition-colors ${errors.title ? 'border-red-300 bg-red-50' : 'border-gray-300'
                    }`}
              ...
              >
                <input
                  ...
                  type="text"
                  {...register('slug')}
                  className={`w-full px-4 py-2.5 border rounded-lg focus:ring-2 focus:ring-indigo-500 focus:border-indigo-500 transition-colors font-mono text-sm ${errors.slug ? 'border-red-300 bg-red-50' : 'border-gray-300'
                    }`}
            ...
            <div>
              ...
              <Controller
                ...
                render={({ field: { onChange, value } }) => (
                  <ImageUploader
                    multiple={false}
                    initialImages={value?.url ? [{ url: value.url, status: 'success' as const }] : []}
                    onUploadComplete={(images) => {
                      if (images.length > 0) {
                        onChange({ url: images[0].url, alt: 'Banner' });
                      } else {
                        onChange(undefined);
                      }
                    }}
          ...
          <div className="space-y-6">
            <div className="flex items-center justify-between">
              <div>
                ...
                <p className="text-sm text-gray-500">Add service items with descriptions and images</p>
              </div>
              <button
                type="button"
                onClick={() => appendService({ title: '', description: '', order: servicesListFields.length })}
            ...
            {servicesListFields.length === 0 ? (
              <div className="text-center py-12 bg-gray-50 rounded-lg border-2 border-dashed border-gray-300">
                <p className="text-gray-500">No service items yet. Click "Add Service Item" to get started.</p>
              </div>
            ) : (
              <div className="space-y-4">
                {servicesListFields.map((field, index) => (
                  <div key={field.id} className="bg-gray-50 rounded-lg p-5 border border-gray-200">
                    <div className="flex items-center justify-between mb-4">
                      <span className="text-sm font-medium text-gray-700">Service Item #{index + 1}</span>
                      <button
                        type="button"
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                      <FormField id={`servicesList.${index}.title`} label="Title">
                        <input
                          id={`servicesList.${index}.title`}
                          type="text"
                          {...register(`servicesList.${index}.title`)}
                          className="w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-indigo-500 focus:border-indigo-500"
                          placeholder="Service item title"
                        />
                      </FormField>
                      <FormField id={`servicesList.${index}.link`} label="Link (optional)">
                        <input
                          id={`servicesList.${index}.link`}
                          type="text"
                          {...register(`servicesList.${index}.link`)}
                          className="w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-indigo-500 focus:border-indigo-500"
                    ...
                    <div className="mt-4">
                      <FormField id={`servicesList.${index}.description`} label="Description">
                        <textarea
                          id={`servicesList.${index}.description`}
                          {...register(`servicesList.${index}.description`)}
                          rows={3}
                          className="w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-indigo-500 focus:border-indigo-500"
          ...
          <div className="space-y-6">
            <div className="flex items-center justify-between">
              <div>
                ...
                <p className="text-sm text-gray-500">Add key activities and outcomes for this service</p>
              </div>
              <button
                type="button"
                onClick={() => appendActivity({ title: '', description: '', order: keyActivitiesFields.length })}
                className="inline-flex items-center px-4 py-2 bg-indigo-600 text-white text-sm font-medium rounded-lg hover:bg-indigo-700 transition-colors"
              >
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                {keyActivitiesFields.map((field, index) => (
                  <div key={field.id} className="bg-gray-50 rounded-lg p-4 border border-gray-200">
                    ...
                    <input
                      type="text"
                      {...register(`keyActivities.${index}.title`)}
                    ...
                    />
                    <textarea
                      {...register(`keyActivities.${index}.description`)}
                      rows={2}
                      className="w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-indigo-500 focus:border-indigo-500"
        ...
        {activeTab === 'features' && (
          <div className="space-y-6">
            <div className="flex items-center justify-between">
              <div>
                <h3 className="text-lg font-medium text-gray-900">Features / Why Work With Us</h3>
                <p className="text-sm text-gray-500">Add features or reasons to work with you</p>
              </div>
              <button
                type="button"
                  ...
                  <div key={field.id} className="bg-gray-50 rounded-lg p-4 border border-gray-200">
                    ...
                    >
                      <input
                        ...
                        className={`w-full px-3 py-2 border rounded-lg focus:ring-2 focus:ring-indigo-500 focus:border-indigo-500 ${errors.features?.[index]?.title ? 'border-red-300 bg-red-50' : 'border-gray-300'
                          }`}
                    ...
                    <div className="mt-2">
                      ...
                      >
                        <textarea
                          ...
                          rows={2}
                          className={`w-full px-3 py-2 border rounded-lg focus:ring-2 focus:ring-indigo-500 focus:border-indigo-500 ${errors.features?.[index]?.description ? 'border-red-300 bg-red-50' : 'border-gray-300'
                            }`}
              ...
              <div className="bg-white border border-gray-200 rounded-lg p-4 mb-6">
                ...
                <div className="space-y-1">
                  <p className="text-blue-600 text-lg hover:underline cursor-pointer truncate">
                    {watchSeoTitle || watch('title') || 'Page Title'}
                  </p>
                  <p className="text-green-700 text-sm">
                    https://yoursite.com/services/{watch('slug') || 'page-slug'}
                  </p>
                  <p className="text-gray-600 text-sm line-clamp-2">
                    {watchSeoDescription || watch('description') || 'Page description will appear here...'}
                  </p>
                </div>
              </div>
              ...
              <div className="space-y-4">
                ...
                >
                  <input
                    ...
                    {...register('seoTitle')}
                    maxLength={60}
                    className={`w-full px-4 py-2.5 border rounded-lg focus:ring-2 focus:ring-indigo-500 focus:border-indigo-500 ${errors.seoTitle ? 'border-red-300 bg-red-50' : 'border-gray-300'
                      }`}
                    placeholder="SEO optimized title (50-60 characters)"
                  />
                  <div className="mt-1 h-1 bg-gray-200 rounded-full overflow-hidden">
                    <div
                      className={`h-full transition-all ${(watchSeoTitle?.length || 0) > 60
                          ? 'bg-red-500'
                          : (watchSeoTitle?.length || 0) > 50
                            ? 'bg-green-500'
                            : 'bg-yellow-500'
                        }`}
                      style={{ width: `${Math.min(((watchSeoTitle?.length || 0) / 60) * 100, 100)}%` }}
                    />
                  </div>
                </FormField>
                ...
                >
                  <textarea
                    ...
                    maxLength={160}
                    rows={3}
                    className={`w-full px-4 py-2.5 border rounded-lg focus:ring-2 focus:ring-indigo-500 focus:border-indigo-500 ${errors.seoDescription ? 'border-red-300 bg-red-50' : 'border-gray-300'
                      }`}
                    placeholder="SEO optimized description (150-160 characters)"
                  />
                  <div className="mt-1 h-1 bg-gray-200 rounded-full overflow-hidden">
                    <div
                      className={`h-full transition-all ${(watchSeoDescription?.length || 0) > 160
                          ? 'bg-red-500'
                          : (watchSeoDescription?.length || 0) > 150
                            ? 'bg-green-500'
                            : 'bg-yellow-500'
                        }`}
                      style={{ width: `${Math.min(((watchSeoDescription?.length || 0) / 160) * 100, 100)}%` }}
                    />
                  </div>
                </FormField>
              </div>
