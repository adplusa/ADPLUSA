...
const metaTagArrayArbitrary = fc.array(metaTagArbitrary, {
    minLength: 0,
    maxLength: 20,
});
...
describe("Meta Tag Array Operations", () => {
    ...
    it("should decrease array length by one when removing a meta tag", () => {
        ...
        const nonEmptyMetaTagArrayArbitrary = fc.array(metaTagArbitrary, {
            minLength: 1,
            maxLength: 20,
        });
    ...
    it("should preserve array length and other entries when updating a meta tag", () => {
        ...
        fc.assert(
            fc.property(
                ...
                (tags, newValue) => {
                    const indexToUpdate = Math.floor(
                        Math.random() * tags.length,
                    );
                    const originalLength = tags.length;
                    const result = updateMetaTag(
                        tags,
                        indexToUpdate,
                        "name",
                        newValue,
                    ...
                    expect(result[indexToUpdate].content).toBe(
                        tags[indexToUpdate].content,
                    );
        ...
        fc.assert(
            fc.property(
                ...
                (tags, newTag) => {
                    ...
                    const afterRemove = removeMetaTag(
                        afterAdd,
                        afterAdd.length - 1,
                    );
        ...
        fc.assert(
            fc.property(metaTagArrayArbitrary, (tags) => {
                ...
                if (validTags.length === 0) {
                    ...
                    const metaTagCount = metaTagMatches
                        ? metaTagMatches.length
                        : 0;
...
describe("Parse Custom Head Tags", () => {
    ...
    it("should round-trip meta tags through HTML conversion", () => {
        ...
        const simpleMetaTagArrayArbitrary = fc.array(simpleMetaTagArbitrary, {
            minLength: 0,
            maxLength: 10,
        });
