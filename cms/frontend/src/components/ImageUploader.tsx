...
export default function ImageUploader({
  ...
  const uploadSingleFile = useCallback(async (fileState: FileUploadState): Promise<UploadedImage | null> => {
    ...
    try {
      ...
      const uploadedImage: UploadedImage = {
        id: data.id || data._id || '',
        url: data.cloudFrontUrl || data.cdnUrl || data.url || data.s3Url,
  ...
  }, [fileUploads, uploadSingleFile, onUploadComplete, multiple, onImagesChange]);
  ...
  const handleFiles = useCallback(async (files: FileList | null) => {
    ...
    if (successfulUploads.length > 0) {
      setUploadedImages(prev => {
        const newImages = multiple ? [...prev, ...successfulUploads] : successfulUploads;
        if (onImagesChange) onImagesChange(newImages);
        return newImages;
      });
  ...
  }, [multiple, maxFiles, uploadedImages.length, fileUploads, validateFiles, uploadSingleFile, onUploadComplete, announce, onImagesChange]);
  ...
  const performDelete = useCallback(async (image: UploadedImage) => {
    try {
      ...
      setUploadedImages(prev => {
        const newImages = prev.filter(img => img.id !== image.id || img.url !== image.url);
        if (onImagesChange) onImagesChange(newImages);
        return newImages;
      });
