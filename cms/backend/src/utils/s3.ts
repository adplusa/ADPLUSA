...
export async function uploadImageToS3(
  ...
  const { buffer, originalName, folder, maxWidth, maxHeight, quality } = options;
  ...
  // Optimize image
  const { buffer: optimizedBuffer, width, height, format } = await optimizeImage(
    buffer,
    maxWidth,
    maxHeight,
    quality
  ...
  if (optimizedBuffer.length === 0) {
    throw new Error('Image optimization failed: Output buffer is empty');
  }
...
export async function generateMultiplePresignedUploadUrls(
  ...
  const presignedUrls = await Promise.all(
    files.map((file) =>
      generatePresignedUploadUrl(folder, file.fileName, file.contentType, expiresIn)
    )
  );
