...
export const updateProject = async (
    ...
    try {
        ...
        // Invalidate Cache
        // Invalidate the new slug (and list)
        await CacheService.invalidateProject(project.slug);

        // If slug changed, invalidate the old slug as well to remove stale data
        if (originalSlug && originalSlug !== project.slug) {
            console.log(`🔥 Invalidating Old Project Slug: ${originalSlug}`);
            await CacheService.invalidateProject(originalSlug);
        }
