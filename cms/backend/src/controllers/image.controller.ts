    ...
    if (!isS3Configured()) {
      res.status(503).json({
        ...
        error: {
          ...
          message: 'AWS S3 is not properly configured. Please check environment variables.',
        },
      });
      return;
    }
...
export async function uploadMultipleImages(req: Request, res: Response): Promise<void> {
  try {
    ...
    // Upload to S3
    const uploadResults = await uploadMultipleImagesToS3(filesToUpload, folder);
 ...
 */
export async function deleteMultipleImages(req: Request, res: Response): Promise<void> {
  try {
    const { ids } = req.body;
