...
export const projectValidationSchema: ValidationSchema = {
  ...
  slug: {
    ...
    type: 'string',
    pattern: /^[a-z0-9-]+$/,
    message: 'Slug is required and can only contain lowercase letters, numbers, and hyphens',
  },
  description: {
    required: false,
    type: 'string',
...
export const userRegistrationSchema: ValidationSchema = {
  username: {
    ...
    maxLength: 50,
    pattern: /^[a-zA-Z0-9_-]+$/,
    message: 'Username is required and must be 3-50 characters (letters, numbers, underscore, hyphen only)',
  },
  email: {
    required: true,
    type: 'string',
  ...
  role: {
    required: false,
    type: 'string',
    custom: (value) => ['admin', 'editor'].includes(value) || 'Role must be either admin or editor',
  },
};
