...
async function migrateHomepage() {
    ...
    if (contactRef) {
        contactImageUrl = await migrateImage(
            contactRef,
            "homepage",
            "contact-image.jpg",
        );
    ...
    const homepage = {
        ...
        contactImage: contactImageUrl
            ? { url: contactImageUrl, alt: "Contact" }
            : undefined,
    };
...
async function migrateProjects() {
    ...
    console.log(
        `  Found ${listingData?.projects?.length || 0} projects in listing`,
    );
    ...
    for (let i = 0; i < internalPages.length; i++) {
        ...
        if (page.moreContent && Array.isArray(page.moreContent)) {
            ...
            if (contentItems.length > 0) {
                ...
                if (
                    firstItem.toLowerCase().includes("scope") ||
                    firstItem.toLowerCase().includes("work")
                ) {
        ...
        await Project.create({
            ...
            images: allImages, // All images consolidated into one array
            imageGalleries: [], // Empty - images are now in 'images' array for CMS editability
            projectDetails,
            category: "",
            featured: isFeatured,
        ...
        stats.projects++;
        console.log(
            `  ✓ ${title} (${baseSlug})${isFeatured ? " [featured]" : ""}`,
        );
    }
}

// ============================================
// PROJECTS PAGE MIGRATION (Singleton)
// ============================================
async function migrateProjectsPage() {
    console.log("\n📄 Migrating Projects Page (Singleton)...");
    ...
    const data = await sanityClient.fetch(query);
    if (!data) {
        console.log("  No Projects Page found in Sanity");
        return;
    }
    ...
    await ProjectsPage.create({
        pageTitle: data.heading || "Our Projects",
        heading: data.heading || "Our Projects",
        seoTitle: truncateSEO(data.seoTitle || "Our Projects", 60),
        seoDescription: truncateSEO(data.seoDescription || "", 160),
    });

    stats.projectsPage = true;
    console.log("  ✓ Projects Page migrated");
}
...
async function migrateServices() {
    ...
    // Fetch Homepage data to get correct service images
    ...
    // Create a map of Service URL/Title -> Image
    const homepageServiceImageMap = new Map();
    if (homepageData?.serviceBox) {
        for (let i = 0; i < homepageData.serviceBox.length; i++) {
            const box = homepageData.serviceBox[i];
            const ref =
                box.serviceBoxImg?.asset?._id || box.serviceBoxImg?.asset?._ref;

            // Map by transformed URL (e.g. /services/drafting-to-cad)
            const url = transformServiceUrl(box.boxUrl);
            const slug = url.split("/").pop(); // "drafting-to-cad"

            if (slug && ref) {
                // Pre-migrate this image
                ...
                if (imageUrl) {
                    homepageServiceImageMap.set(slug, {
                        url: imageUrl,
                        alt: box.serviceBoxTitle,
                    });
                }
            }
        }
    ...
    if (mainData) {
        const title =
            mainData.whyWorkWithUs?.title || mainData.title || "Our Services";
        const slug = "main-services";

        let bannerUrl = "";
        const bannerRef =
            mainData.serviceBannerImage?.asset?._ref ||
            mainData.serviceBannerImage?.asset?._id;
        if (bannerRef) {
            bannerUrl = await migrateImage(
                bannerRef,
                "services/main",
                "banner.png",
            );
        ...
        let mainImageUrl = "";
        const mainRef =
            mainData.whyWorkWithUs?.image?.asset?._ref ||
            mainData.whyWorkWithUs?.image?.asset?._id;
        if (mainRef) {
            mainImageUrl = await migrateImage(
                mainRef,
                "services/main",
                "main.jpg",
            );
    ...
    for (let i = 0; i < allServices.length; i++) {
        ...
        const slug =
            SERVICE_TYPE_SLUG_MAP[service._type] ||
            service._type.replace(/Page$/, "").toLowerCase();
        const title = service.title || `Service ${i + 1}`;

        // Migrate banner image
        let bannerUrl = "";
        const bannerRef =
            service.serviceBannerImage?.asset?._ref ||
            service.serviceBannerImage?.asset?._id;
        if (bannerRef) {
            bannerUrl = await migrateImage(
                bannerRef,
                `services/${slug}`,
        ...
        // Determine listing image: Prefer Homepage image, fallback to internal banner
        let listingImage = homepageServiceImageMap.get(slug);
        if (!listingImage && bannerUrl) {
        ...
        await Service.create({
            ...
            bannerImage: bannerUrl
                ? { url: bannerUrl, alt: `${title} Banner` }
                : undefined,
            image: listingImage, // Use correct image from Homepage mapping
            servicesList,
            keyActivities,
            features,
...
// ============================================
// MAIN SERVICE PAGE MIGRATION (Singleton)
// ============================================
async function migrateMainServicePage() {
    console.log("\n📄 Migrating Main Service Page (Singleton)...");

    // Using serviceTwoPage as the source for Main Service Page content as identified
    ...
    if (!data) {
        console.log(
            "  No Main Service Page data found in Sanity (serviceTwoPage)",
        );
        return;
    }

    await MainServicePage.deleteMany({});

    // Migrate Banner Image
    let bannerUrl = "";
    const bannerRef =
        data.serviceBannerImage?.asset?._ref ||
        data.serviceBannerImage?.asset?._id;
    if (bannerRef) {
        bannerUrl = await migrateImage(
            bannerRef,
            "main-service-page",
    ...
    // Migrate Why Work With Us Image
    let whyWorkImage = "";
    const whyWorkRef =
        data.whyWorkWithUs?.image?.asset?._ref ||
        data.whyWorkWithUs?.image?.asset?._id;
    if (whyWorkRef) {
        whyWorkImage = await migrateImage(
            whyWorkRef,
            "main-service-page",
            "why-work.jpg",
        );
    ...
    // Map features to whyWorkWithUsItems
    const whyWorkWithUsItems = (data.whyWorkWithUs?.features || []).map(
        (feature: any) => ({
            icon: feature.icon || "",
            title: feature.title || "",
    ...
    await MainServicePage.create({
        bannerImage: bannerUrl
            ? { url: bannerUrl, alt: "Main Service Banner" }
            : undefined,
        bannerTitle: data.title || "Our Services",
        pageTitle: data.title || "Our Services",
        showTrustIcons: true,
        trustIconsHeading: "Trusted By",
        servicesHeading: "All Services",
        showWhyWorkWithUs: true,
        whyWorkWithUsHeading: data.whyWorkWithUs?.title || "Why Work With Us",
        whyWorkWithUsItems,
        whyWorkWithUsImage: whyWorkImage
            ? { url: whyWorkImage, alt: "Why Work With Us" }
            : undefined,
        showContactForm: true,
        contactFormHeading: "Get In Touch",
        contactFormSubheading: "We are here to help you",
        seoTitle: truncateSEO(data.seoTitle || data.title || "Services", 60),
        seoDescription: truncateSEO(data.seoDescription || "", 160),
    });

    stats.mainServicePage = true;
    console.log("  ✓ Main Service Page migrated");
}
...
async function migrateContact() {
    ...
    let contactImageUrl = "";
    const contactImageRef =
        data.contactImage?.asset?._ref || data.contactImage?.asset?._id;
    if (contactImageRef) {
        contactImageUrl = await migrateImage(
            contactImageRef,
            "contact",
    ...
    let rightImageUrl = "";
    const rightImageRef =
        data.rightImage?.asset?._ref || data.rightImage?.asset?._id;
    if (rightImageRef) {
        rightImageUrl = await migrateImage(
            rightImageRef,
            "contact",
            "right-image.jpg",
        );
    ...
    await Contact.create({
        ...
        formFields,
        contactImage: contactImageUrl
            ? { url: contactImageUrl, alt: "Contact Image" }
            : undefined,
        contactInfo: {
        ...
        rightImage: rightImageUrl
            ? { url: rightImageUrl, alt: "Why Work With Us" }
            : undefined,
...
// ============================================
async function migrateGeneralSettings() {
    console.log("\n📄 Migrating General Settings...");
    ...
    const query = `*[_type == "generalSettings"][0]{
        ...
        contactEmail,
        footerText,
        headerLogo{asset->{_id, url}},
        footerLogo{asset->{_id, url}},
    ...
    // If no Sanity data, use local assets from the public folder
    if (!data) {
        console.log(
            "  No General Settings found in Sanity, using local assets...",
        );

        // Upload local assets to S3
        // These files exist in /public folder of the Next.js app
        const localAssets = {
            headerLogo: path.join(process.cwd(), "../../public/red-logo.png"),
            footerLogo: path.join(
                process.cwd(),
        ...
        // Upload header logo
        try {
            if (fs.existsSync(localAssets.headerLogo)) {
                const fileBuffer = fs.readFileSync(localAssets.headerLogo);
                const result = await uploadImageToS3({
                    buffer: fileBuffer,
                    originalName: "header-logo.png",
                    contentType: "image/png",
                    folder: "settings",
                });
                headerLogoUrl = result.cdnUrl;
                console.log("    ✓ Uploaded header logo from local assets");
            } else {
                console.log(
                    "    ⚠ Header logo not found at:",
                    localAssets.headerLogo,
                );
            }
        } catch (err) {
            console.log("    ⚠ Failed to upload header logo:", err);
        }

        // Upload footer logo
        try {
            if (fs.existsSync(localAssets.footerLogo)) {
                const fileBuffer = fs.readFileSync(localAssets.footerLogo);
                const result = await uploadImageToS3({
                    buffer: fileBuffer,
                    originalName: "footer-logo.png",
                    contentType: "image/png",
                    folder: "settings",
                });
                footerLogoUrl = result.cdnUrl;
                console.log("    ✓ Uploaded footer logo from local assets");
            } else {
                console.log(
                    "    ⚠ Footer logo not found at:",
                    localAssets.footerLogo,
                );
            }
        } catch (err) {
            console.log("    ⚠ Failed to upload footer logo:", err);
        }

        // Upload favicon
        try {
            if (fs.existsSync(localAssets.favicon)) {
                const fileBuffer = fs.readFileSync(localAssets.favicon);
                const result = await uploadImageToS3({
                    buffer: fileBuffer,
                ...
                faviconUrl = result.cdnUrl;
                console.log("    ✓ Uploaded favicon from local assets");
            } else {
                console.log("    ⚠ Favicon not found at:", localAssets.favicon);
            }
        } catch (err) {
            console.log("    ⚠ Failed to upload favicon:", err);
        }

        await GeneralSettings.create({
            siteTitle: "ADPL Consulting",
            siteDescription:
                "ADPL CONSULTING LLC works as a leading Architectural and Engineering outsource fraternity across India and the United States of America.",
            contactEmail: "info@adplconsulting.com",
            footerText: "© 2025 ADPL Consulting. All rights reserved.",
            headerLogo: headerLogoUrl
                ? { url: headerLogoUrl, alt: "ADPL Consulting Logo" }
                : undefined,
            footerLogo: footerLogoUrl
                ? { url: footerLogoUrl, alt: "ADPL Consulting Footer Logo" }
                : undefined,
            favicon: faviconUrl ? { url: faviconUrl } : undefined,
        });

        stats.generalSettings = true;
        console.log("  ✓ General Settings created from local assets");
        return;
    }

    // Migrate from Sanity data
    // Migrate Header Logo
    let headerLogoUrl = "";
    const headerRef =
        data.headerLogo?.asset?._id || data.headerLogo?.asset?._ref;
    if (headerRef) {
        headerLogoUrl = await migrateImage(
            headerRef,
            "settings",
            "header-logo.png",
        );
    ...
    let footerLogoUrl = "";
    const footerRef =
        data.footerLogo?.asset?._id || data.footerLogo?.asset?._ref;
    if (footerRef) {
        footerLogoUrl = await migrateImage(
            footerRef,
            "settings",
            "footer-logo.png",
        );
    ...
    // Migrate Favicon
    let faviconUrl = "";
    const faviconRef = data.favicon?.asset?._id || data.favicon?.asset?._ref;
    if (faviconRef) {
        faviconUrl = await migrateImage(faviconRef, "settings", "favicon.png");
    }

    await GeneralSettings.create({
        siteTitle: data.siteTitle || "ADPL Consulting",
        siteDescription: data.siteDescription || "ADPL Consulting",
        contactEmail: data.contactEmail || "info@adplconsulting.com",
        footerText:
            data.footerText || "© 2024 ADPL Consulting. All rights reserved.",
        headerLogo: headerLogoUrl
            ? { url: headerLogoUrl, alt: "Header Logo" }
            : undefined,
        footerLogo: footerLogoUrl
            ? { url: footerLogoUrl, alt: "Footer Logo" }
            : undefined,
        favicon: faviconUrl ? { url: faviconUrl } : undefined,
    });
...
async function main() {
    ...
    try {
        ...
        console.log(`  Projects Page: ${stats.projectsPage ? "✓" : "✗"}`);
        console.log(`  Services: ${stats.services}`);
        console.log(
            `  Main Service Page: ${stats.mainServicePage ? "✓" : "✗"}`,
        );
        console.log(`  FAQ: ${stats.faq ? "✓" : "✗"}`);
        console.log(`  About: ${stats.about ? "✓" : "✗"}`);
        console.log(`  Contact: ${stats.contact ? "✓" : "✗"}`);
        console.log(`  General Settings: ${stats.generalSettings ? "✓" : "✗"}`);
        console.log(
            `  Images: ${stats.images.success} success, ${stats.images.failed} failed`,
        );
        console.log(
            "═══════════════════════════════════════════════════════════\n",
        );
